...
test('enviar solicitud de reseteo con correo válido', async ({ page }) => {
  ...
  await page.fill('input[name="email"]', data.Emailconsulta);

  // Hacer clic en el botón de inicio de sesión
  await page.click('button[type="submit"]');

  // Le indicamos que espere hasta que la pagina contenga 
  await expect(page.locator("//h1[contains(.,'Link enviado!')]")).toBeVisible();
  ...
  const screenshotPath = path.join(carpetaBase, 'solicitud_de_reseteo_con_correo_válido.png');
