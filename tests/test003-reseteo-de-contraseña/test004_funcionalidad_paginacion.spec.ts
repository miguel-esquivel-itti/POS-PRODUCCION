...
test('volver a Inicio de sesión', async ({ page }) => {
  ...
  // Hacer clic en el botón de olvide contraseña
  await page.click('//button[contains(.,"Volver a Inicio de sesión")]');

  // Le indicamos que espere hasta que la pagina contenga 
  await expect(page.locator('//span[contains(.,"Recordar usuario")]')).toBeVisible();
  ...
  const screenshotPath = path.join(carpetaBase, 'volver_al_inicio_de_sesión.png');
