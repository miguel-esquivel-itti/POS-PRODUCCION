...
test('Ingreso de solo el usuario', async ({ page }) => {
  ...
  await expect(page.locator('text="Contraseña es obligatoria"')).toBeVisible({ timeout: 5000 });
  ...
  const screenshotPath = path.join(carpetaBase, 'Ingreso_de_solo_el_email.png');
