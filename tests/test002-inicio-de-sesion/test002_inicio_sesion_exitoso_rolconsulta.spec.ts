...
test('inicio de sesión exitoso rolconsulta', async ({ page }) => {
  ...
  // Le indicamos que espere hasta que la pagina contenga 
  await expect(page.locator('//span[contains(.,"Consulta")]')).toBeVisible({ timeout: 5000 });
    ...
    const screenshotPath = path.join(carpetaBase, 'inicio_de_sesión_exitoso_rolconsulta.png');
