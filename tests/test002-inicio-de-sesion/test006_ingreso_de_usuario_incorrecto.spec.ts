...
test('Ingreso de usuario incorrecto', async ({ page }) => {
  ...
  await expect(page.locator('text=Su correo electrónico o contraseña son incorrectos')).toBeVisible({ timeout: 5000 });
  ...
  const screenshotPath = path.join(carpetaBase, 'Ingreso_de_usuario_incorrecto.png');
